package com.connxun.elinetv.shortvideo;
...
//        mediaRecordPara.setAppKey("48ff43fa171120daec40375eddcbfb06");  //APPKEY
        mediaRecordPara.setAppKey("4727023efa991d31d61b3b32e819bd5b");  //APPKEY
    ...
    public void startPreview(NeteaseView videoView) {
        ...
        mediaRecord.setFilterStrength(0.9f); //滤镜强度
        mediaRecord.setFilterType(VideoEffect.FilterType.tender);
        isPreviewing = true;
    }

    //设置磨皮
    public void setBeautyLevel(int param){
        if(mediaRecord != null){
            mediaRecord.setBeautyLevel(param); //磨皮强度为5,共5档，0为关闭
        }
    }
    //设置滤镜强度
    public void setFilterStrength(float param){
        if(mediaRecord != null){
            mediaRecord.setFilterStrength(param); //磨皮强度为5,共5档，0为关闭
        }
    }


    public void setFilterType(String param){
        if(mediaRecord != null){
            switch (param)
            {
                case "无滤镜":
                ...
                case "黑白":
                    break;
                case "怀旧":
                    mediaRecord.setFilterType(VideoEffect.FilterType.brooklyn);
                    break;
                case "自然":
                    mediaRecord.setFilterType(VideoEffect.FilterType.nature);
                    break;
                default:
                    break;
            }
