...
public class CapturePreviewController extends CapturePreviewContract.CapturePreviewControllerBase implements lsMessageHandler {
    ...
    private static FuVideoEffect mFuEffect; //FU的滤镜
     ...
     * SDK层 直播控制器
     */
    private static lsMediaCapture mLSMediaCapture = null;
...
//            }
    ...
    private void init() {
        ...
        Log.e("sdfdsafdsa","mLSMediaCapture.getSDKVersion();"+mLSMediaCapture.getSDKVersion());
        ...
        if(mLSMediaCapture != null && captureWrapper.isVideo()) {
            mLSMediaCapture.startVideoPreview((NeteaseView) ui.getDisplaySurfaceView(mUseFilter), captureWrapper.getCameraPosition(),
                    mUseFilter, lsMediaCapture.VideoQuality.HIGH, true);
...
//        });
    ...
    public static void setCheekThinning(float param){
//        param = (float)progress/100;
    ...
    public void setFilterStrength(float strength){
        mLSMediaCapture.setFilterStrength(strength);
    }
    public void setBeautyLevel(int level){
        mLSMediaCapture.setBeautyLevel(level);
