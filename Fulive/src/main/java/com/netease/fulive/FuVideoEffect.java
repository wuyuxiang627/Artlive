...
public class FuVideoEffect{
    ...
    private float mFacebeautyColorLevel = 0.0f; // 美白 （0 - 1）
    private float mFacebeautyBlurLevel = 6.0f;  //磨皮 （1-6）
    private float mFacebeautyCheeckThin = 0f; // 瘦脸 (0 - 2)
    private float mFacebeautyEnlargeEye = 1.0f; //大眼 (0 -4 )
    private int mFaceShape = 0;
    private float mFaceShapeLevel = 1f;
...
//        faceunity.fuItemSetParam(mFacebeautyItem, "face_shape_level", 1); //程度
//        faceunity.fuItemSetParam(mFacebeautyItem, "cheek_thinning", 1); //瘦脸
    }

    public void setCOlorLevel(float param){

        mFacebeautyColorLevel = param;
    }
    public void setCheekThinning(float param){

        mFacebeautyCheeckThin = param;
    }

    public void setFilterName(int position){
